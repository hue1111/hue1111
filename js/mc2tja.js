...
var mc2tja = function() {
    ...
    this.standardTja = true; // set to true to disable tja features that is only supported by malody
...
(function() {
    mc2tja.prototype = {
        ...
        convert: function(mc, onsuccess) {
            ...
            if (mc.initTime) {
                tja.prop('BPM', Number((mc.initTime.bpm).toFixed(3)));
            }
            if (mc.mainSample) {
                ...
                tja.prop('DEMOSTART', '');
				tja.prop('MAKER', mc.meta.creator+'\n');
            }
            ...
            while (noteIndex < notes.length || lastLongNote) {
                ...
                for (; bpmIndex < bpms.length; ++bpmIndex) {
                    ...
                    segmentEvents.push({beat: bpm.beat.cutoff(barBeat), event: new TJAEvent(0, 'BPMCHANGE', Number((bpm.bpm).toFixed(3)))});
                }
                ...
                for (; effectIndex < effects.length; ++effectIndex) {
                    ...
                    if ("hs" in effect) segmentEvents.push({beat: beat, event: new TJAEvent(0, 'SCROLL', Number((effect.hs).toFixed(5)))});
